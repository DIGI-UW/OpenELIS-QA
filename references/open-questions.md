...
| 4 | Test Catalog: Copy configuration from test semantics | Copy never overwrites a configured component; it only appends components whose code is new. Every test has PRIMARY, so the copy changes nothing for 162 of 164 tests, yet toasts "Configuration copied." OGC-967/930 ACs asked for preview + Replace or Append. | (a) implement Replace/Append per the ACs; (b) keep append-only but report "0 copied: PRIMARY already configured" | testing 3.2.2.0 (develop 95d6c64) | TC-SA-02 asserts Replace per the AC; if (b) is chosen it must be rewritten to assert the honest message instead |
...
| 7 | Panel description uniqueness (Delta-SA6) | Saving a panel whose description matches another panel's answers 500 with no visible message | (a) descriptions must be unique: return 409 with a message; (b) they need not be: drop the constraint | testing 3.2.2.0 | decides between a validation tripwire and a schema fix |
