...
| 4 | Home-dashboard Domain filtering | Order entry is already domain-scoped (3 routes: CLINICAL/ENVIRONMENTAL/VECTOR) and lab units are gaining domain scoping via the incoming test-catalog work, but the home dashboard KPI cards (Suite I, `TC-*-KPI-*`) render one aggregate set with no domain filter/toggle observed. | (a) dashboard is intentionally domain-agnostic (aggregate across all domains is the design); (b) a domain filter is expected but not yet built — genuine gap; (c) filtering is meant to follow the logged-in user's lab-unit domain automatically and that binding is broken | testing v3.2.x (any distro running multiple domains) | project notes call this an "UNDOCUMENTED gap" already — deciding (a) vs (b)/(c) determines whether the existing KPI cases should assert "no domain filter" as PASS or whether this is a real missing feature to log |
