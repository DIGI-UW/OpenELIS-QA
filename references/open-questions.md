...
| 8 | Analyzer Error Dashboard: still a product surface? | `/analyzers/errors` is still routed in `App.jsx`, but it has no menu entry (the Analyzers menu lists only List, Types, QC), and a direct visit ends on a blank `/api/OpenELIS-Global/analyzers/errors`. The old BM-DEEP target `/MasterListsPage/AnalyzerTestName` was deliberately removed (develop `AdminSideNav.test.jsx` asserts it), and BM-DEEP-02 was passing on that blank page. | (a) the page is retired: delete the route and keep BM-DEEP retired; (b) it is meant to be live: restore a menu entry and fix the direct-URL landing, then re-author BM-DEEP against it | testing 3.2.2.0 | decides whether BM-DEEP comes back and whether the direct-URL landing is a bug |
