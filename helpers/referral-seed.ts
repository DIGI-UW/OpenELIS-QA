 ...
 *
 * IDEMPOTENT. Re-running finds its own fixtures by name and does nothing. The
 * organizations are matched on `organizationName`; the referrals are matched by
 * reading `.auth/referral-seed.json` back and re-reading each accession through
 * `GET /rest/order/search?labNumber=`, so a re-run against the same instance is a
 * no-op even if the state file is stale.
 *
    ...
    async (args: {
      ...
      for (const view of ['outstanding', 'returned', 'history']) {
        for (const row of (await getJson(`${R}/reference-lab-results/referrals?view=${view}`)) || []) {
          ...
          if (m) alreadyThere.add(m[1]);
        }
      }

      const referrals: any[] = [];
      for (const step of args.plan) {
        // A DRAFT referral appears in no dashboard view, so it cannot be detected this
        // way and is re-created on every run. That is deliberate and cheap: one order.
        // Everything that IS visible is skipped.
        if (alreadyThere.has(step.tag)) {
          referrals.push({
            tag: step.tag,
