 ...
 *
 * This was a per-run stamped name, so TC-ANZ-M3-05 minted a fresh analyzer on every execution.
 * There is NO delete path — DELETE on an analyzer answers 405 — so they accumulate forever: 11 by
 * the end of 2026-08-27, every one stuck on missing-required-values: port.
 *
 * That was not harmless clutter. It reordered the analyzer list and broke TC-ANZ-M3-13, -14 and
 * -15, which selected fixtures by index. Three tests reported schema and probe regressions that did
 * not exist.
 *
 * So: seed once, reuse thereafter. (From the parallel re-baseline, 530de0a.)
 */
...
async function probeableAnalyzer(page: Page): Promise<any | null> {
  // Never select a QA-created row. Even now that seeding has stopped adding to them, the
  // ones already on the instance must not be chosen as a fixture. (530de0a.)
...
test.describe('TC-ANZ-M3-INSTRUMENT — inline instrument-first setup (FR-B1, B2, B3)', () => {
  ...
  test('TC-ANZ-M3-03 the type picker filters as it searches [AC-3 · FUNCTION]', async ({ page }) => {
    ...
    // Δ-W FIXED 2026-08-27 for this control. This assertion is a GUARD, not a flip-when-fixed —
    // if it fails, shouldFilterItem was dropped from the Carbon ComboBox again.
    //
    // MEASURED 2026-09-01, at 4 options and again at 13, to settle three conflicting readings
    // ("does not filter", "filters by PREFIX", "the search empties the menu 13→0"). The control is
    // a CASE-INSENSITIVE SUBSTRING filter over the whole option label:
    //
    //     "Fluoro" → 1     mid-string in "Bruker FluoroCycler XT"   (prefix would give 0)
    //     "Cycler" → 1     mid-string                               (prefix would give 0)
    //     "Fisher" → 1     mid-string in "Thermo Fisher QuantStudio"(prefix would give 0)
    //     "bruker" → same as "Bruker"                               (case-insensitive)
    //     "zzz"    → 0
    //
    // So it is NOT prefix-anchored, and it does not empty the menu: at 13 options both `fill('')`
    // and select-all-then-type gave 13 → 1. See probe-typepicker.spec.ts.
    //
    // "Fluo" is deliberately narrow: the seed creates site-derived GeneXpert duplicates of its
    // own, so any GeneXpert-shaped query matches a drifting number of rows and cannot tell
    // "no filtering" from "filtering, and nearly everything matches".
  ...
  test('TC-ANZ-M3-05 create from a clean list POSTs a new analyzer and it round-trips [FR-B2 · ROUND-TRIP]', async ({ page }) => {
    await login(page);
    // Seed once. If the fixture is already here, exercise the round-trip against it rather than
    // minting another orphan we have no way to remove.
    ...
    console.log(
      `${existing ? '[fixture] reusing analyzer ' : '[fixture] seeded analyzer '}${id} ` +
        `(${ANALYZER_NAME}) — no delete path exists, so we never accumulate`,
    );
    ...
    // A freshly seeded analyzer is in SETUP. A reused one may have been walked through activation
    // by TC-ANZ-M3-16, so accept any lifecycle state rather than forcing a fresh create just to
    // keep one assertion literal.
