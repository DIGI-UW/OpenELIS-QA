...
description: >
  Automated QA testing skill for OpenELIS Global covering 148+ test suites and ~323 test cases across the full application. Tests: Orders, Validation, Results, Patient Management, Dashboard, Admin (28+ pages), Reports (all 11), Referrals, Workplan, FHIR, i18n, Accessibility, Pathology, Analyzers, EQA, Alerts, Storage, Batch Entry, Barcode, and more. Includes DEEP interaction suites testing search, filter, form interaction, error handling, performance, cross-module data integrity, security (CSRF/XSS/SQLi), WCAG accessibility, end-to-end order tracing, and report PDF generation. Drives a real browser session via Claude in Chrome and produces a pass/fail report with Jira tickets for failures.
---
...
**Cumulative:** 323 TCs executed, 302 passed, ~93% pass rate (3 resolved bugs + 1 retracted false positive improve effective quality).
