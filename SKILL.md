...
| BUG-46 | Low | **FIXED in v3.2.1.4 (verified Phase 40)** ~~`/MasterListsPage/calculatedValue` admin page calls `GET /rest/calculatedValue` and `GET /rest/testCalculatedValue` on every page load — both return HTTP 404.~~ Cannot reproduce in v3.2.1.4 — only correct endpoint `GET /rest/test-calculations` (200) observed on fresh page load. | Admin — Calculated Value Tests Management |
