...
| BUG-55 | High | **FIXED (Phase 54 — v3.2.1.5)** ~~NEW (Phase 44 — v3.2.1.5)~~ React Router basename misconfiguration caused `/Workplan` and `/NonConformingEvent` top-level routes to redirect internally to `/api/OpenELIS-Global/[path]`. **CONFIRMED FIXED** in Phase 54 retesting (2026-04-13): all WorkPlanBy* routes (`/WorkPlanByTest`, `/WorkPlanByPanel`, `/WorkPlanByTestSection`, `/WorkPlanByPriority`) now load correct pages without redirect. Fix deployed to mgtest intraday between Phase 44 and Phase 54 testing. | Core Workflows — Workplan, Non-Conforming Event |
