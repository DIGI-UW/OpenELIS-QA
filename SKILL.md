...
| BUG-50 | Medium | **NEW (Phase 42 — v3.2.1.5)** `GET /rest/provider` → HTTP 404 in v3.2.1.5. The `/MasterListsPage/providerMenu` page renders but degrades to showing only 1 row ("UNKNOWN_" with isActive=false, no name/phone/fax). Real provider data cannot be loaded or displayed. Was working (200) in v3.2.1.4. | Admin — Provider Management |
| BUG-51 | High | **NEW (Phase 42 — v3.2.1.5)** `GET /rest/dictionary` → HTTP 404 in v3.2.1.5. The DictionaryMenu admin page likely renders blank or shows no dictionary entries. Was working in v3.2.1.4. | Admin — Dictionary Menu |
| BUG-52 | High | **NEW (Phase 42 — v3.2.1.5)** `GET /rest/patient/search` → HTTP 404 in v3.2.1.5. Patient search throughout the application (PatientManagement page, Edit Order, Result lookup by patient) returns no results because the search endpoint is not deployed. | Patient Management — Search |
| BUG-53 | Medium | **NEW (Phase 42 — v3.2.1.5)** `GET /rest/referrals` → HTTP 404 in v3.2.1.5. The Referred Out Tests page (`/ReferredOutTests`) cannot load referred test data. Referral lookup is broken. Was working in v3.2.1.4. | Results — Referred Out Tests |
| BUG-54 | Medium | **NEW (Phase 42 — v3.2.1.5)** `GET /rest/calculatedValue` and `GET /rest/testCalculatedValue` → HTTP 404 in v3.2.1.5 (see BUG-46 regression note). Additionally `GET /rest/organizationManagement/providers` → 404, breaking Organization Management admin page. | Admin — Calculated Values, Organization Management |
| BUG-55 | High | **NEW (Phase 44 — v3.2.1.5)** React Router basename misconfiguration causes `/Workplan` and `/NonConformingEvent` (and `/Report`) top-level routes to redirect internally to `/api/OpenELIS-Global/[path]` at runtime → Spring `NoHandlerFoundException` 404. Server HEAD requests confirm all routes return HTTP 200 (server correctly serves SPA shell); the bug is in the client-side router. Workplan and Non-Conforming Event pages completely inaccessible via React SPA navigation. | Core Workflows — Workplan, Non-Conforming Event, Reports |
| BUG-56 | Medium | **NEW (Phase 45 — v3.2.1.5)** FHIR stack not deployed on mgtest. `GET /fhir/metadata` returns `text/html` (SPA shell, HTTP 200) instead of a FHIR R4 CapabilityStatement JSON. Working correctly on testing v3.2.1.4 (HAPI FHIR 7.0.2, R4). All FHIR resource endpoints expected to be non-functional. | FHIR — Metadata, Patient, Observation |
| BUG-57 | Medium | **NEW (Phase 45 — v3.2.1.5)** `/Report?type=patient` redirects to `/` (Dashboard). Same React Router basename issue as BUG-55. Additionally, all `/rest/report/*` sub-endpoints return HTTP 404 — Reports module entirely inaccessible on mgtest. Was functional on testing v3.2.1.4. | Reports — All report types |
