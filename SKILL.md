...
| BUG-46 | Low | **NEW (Phase 39C)** `/MasterListsPage/calculatedValue` admin page calls `GET /rest/calculatedValue` and `GET /rest/testCalculatedValue` on every page load — both return HTTP 404 `NoHandlerFoundException`. The correct endpoint is `GET /rest/test-calculations` (returns 200 with full rule data). Page renders correctly because it also calls the correct endpoint, but 2 spurious 404 errors occur on every load. | Admin — Calculated Value Tests Management |
| BUG-47 | Low | **NEW (Phase 39A)** "Receive New Inventory Lot" modal (opened via "Add New Lot" button on `/Inventory`) displays raw i18n key `storage.location.add.button` as a button label inside the Storage Location field. Expected: human-readable label (e.g., "Add Storage Location"). All other modal fields render correctly. | Inventory Management — Add New Lot Modal |
